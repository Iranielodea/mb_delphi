...
var
  fIReceber: TfIReceber;

implementation

uses uDM, uCliente, uForma_Pagto, UUtil, uContaBanco, uRContas;

{$R *.dfm}

procedure TfIReceber.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   action:=cafree;
end;

procedure TfIReceber.PesquisaCliente(Tipo: String);
var vreg: integer;
begin
...
procedure TfIReceber.Cod_ClienteExit(Sender: TObject);
begin
   if Cod_Cliente.Modified then
      PesquisaCliente('C');
end;

procedure TfIReceber.NomeExit(Sender: TObject);
begin
   if Nome.Modified then
      PesquisaCliente('D');
end;

procedure TfIReceber.NomeEnter(Sender: TObject);
begin
   Nome.Modified:=false;
end;

procedure TfIReceber.PesquisaForma_Pagto(Tipo: String);
var vreg: integer;
begin
...
procedure TfIReceber.Cod_PagtoExit(Sender: TObject);
begin
   if Cod_Pagto.Modified then
      PesquisaForma_Pagto('C');
end;

procedure TfIReceber.Desc_PagtoExit(Sender: TObject);
begin
   if Desc_Pagto.Modified then
      PesquisaForma_Pagto('D');
end;

procedure TfIReceber.Cod_PagtoEnter(Sender: TObject);
begin
   Cod_Pagto.Modified:=false;
end;

procedure TfIReceber.Desc_PagtoEnter(Sender: TObject);
begin
   Desc_Pagto.Modified:=false;
end;

procedure TfIReceber.PesquisaConta(Tipo: String);
var vreg: integer;
sAtivo: string;
begin
...
procedure TfIReceber.Id_ContaBancoExit(Sender: TObject);
begin
   if Id_ContaBanco.Modified then
      PesquisaConta('C');
end;

procedure TfIReceber.Num_ContaExit(Sender: TObject);
begin
   if Num_Conta.Modified then
      PesquisaConta('D');
end;

procedure TfIReceber.Num_ContaEnter(Sender: TObject);
begin
   Num_Conta.Modified:=false;
end;

procedure TfIReceber.Id_ContaBancoEnter(Sender: TObject);
begin
   Id_ContaBanco.Modified:=false;
end;

procedure TfIReceber.SpeedButton3Click(Sender: TObject);
begin
    fContaBanco:=TfContaBanco.create(true);
    fContaBanco.TipoCad.Text:='P';
    if fContaBanco.ShowModal = mrOk then
...
procedure TfIReceber.SpeedButton1Click(Sender: TObject);
begin
    fForma_Pagto:=TfForma_Pagto.create(Self);
    fForma_Pagto.TipoCad.Text:='P';
    if fForma_Pagto.ShowModal = mrOk then
...
procedure TfIReceber.SpeedButton2Click(Sender: TObject);
begin
    fCliente:=TfCliente.create(Self);
    fCliente.TipoCad.Text:='P';
    if fCliente.ShowModal = mrOk then
...
procedure TfIReceber.butSairClick(Sender: TObject);
begin
   close;
end;

procedure TfIReceber.FormKeyPress(Sender: TObject; var Key: Char);
begin
   if key = #13 then
   begin
      key:=#0;
...
procedure TfIReceber.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   case key of
      vk_Escape: close;
...
procedure TfIReceber.butImprimirClick(Sender: TObject);
var
   vsql: string;
   vlData: String;
...
//------------------------------------------------------------------------------
   if Cod_Cliente.Text <> '' then
      vsql:=vsql + ' and CON.COD_CLIENTE = '+Cod_Cliente.Text;
...
//------------------------------------------------------------------------------
   frContas:=TfrContas.create(self);
   with frContas do
   begin
      Receber.Close;
...
procedure TfIReceber.FormCreate(Sender: TObject);
begin
