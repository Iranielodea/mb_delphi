object fListaVenda: TfListaVenda
  Left = 29
  Top = 81
  Caption = 'Lista de Vendas'
  ClientHeight = 483
  ClientWidth = 878
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 16
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 878
    Height = 243
    Align = alTop
    BevelOuter = bvLowered
    TabOrder = 0
    object Label1: TLabel
      Left = 8
      Top = 16
      Width = 48
      Height = 16
      Caption = 'Per'#237'odo'
    end
    object Label2: TLabel
      Left = 168
      Top = 16
      Width = 20
      Height = 16
      Caption = 'At'#233
    end
    object Label6: TLabel
      Left = 448
      Top = 16
      Width = 283
      Height = 16
      Caption = 'Bot'#227'o direito na grade para Outras Informa'#231#245'es'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label7: TLabel
      Left = 8
      Top = 32
      Width = 70
      Height = 16
      Caption = 'Fornecedor'
    end
    object Label8: TLabel
      Left = 376
      Top = 32
      Width = 41
      Height = 16
      Caption = 'Cliente'
    end
    object Label9: TLabel
      Left = 8
      Top = 72
      Width = 46
      Height = 16
      Caption = 'Contato'
    end
    object SpeedButton1: TSpeedButton
      Left = 338
      Top = 48
      Width = 23
      Height = 22
      Glyph.Data = {
        
        00FF9C948C9C948C9C948C9C948CFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      OnClick = SpeedButton1Click
    end
    object SpeedButton2: TSpeedButton
      Left = 706
      Top = 48
      Width = 23
      Height = 22
      Glyph.Data = {
        
        00FF9C948C9C948C9C948C9C948CFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      OnClick = SpeedButton2Click
    end
    object SpeedButton3: TSpeedButton
      Left = 338
      Top = 88
      Width = 23
      Height = 22
      Glyph.Data = {
        
        00FF9C948C9C948C9C948C9C948CFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      OnClick = SpeedButton3Click
    end
    object Label10: TLabel
      Left = 376
      Top = 72
      Width = 55
      Height = 16
      Caption = 'Motorista'
    end
    object SpeedButton4: TSpeedButton
      Left = 706
      Top = 88
      Width = 23
      Height = 22
      Glyph.Data = {
        
        00FF9C948C9C948C9C948C9C948CFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      OnClick = SpeedButton4Click
    end
    object Label11: TLabel
      Left = 192
      Top = 168
      Width = 59
      Height = 16
      Caption = 'Indica'#231#227'o'
      Visible = False
    end
    object SpeedButton5: TSpeedButton
      Left = 338
      Top = 184
      Width = 23
      Height = 22
      Glyph.Data = {
        
        00FF9C948C9C948C9C948C9C948CFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      Visible = False
      OnClick = SpeedButton5Click
    end
    object Shape1: TShape
      Left = 744
      Top = 48
      Width = 17
      Height = 17
      Brush.Color = clRed
    end
    object Label12: TLabel
      Left = 768
      Top = 48
      Width = 119
      Height = 16
      Caption = 'Parcelas em Aberto'
    end
    object Shape2: TShape
      Left = 744
      Top = 72
      Width = 17
      Height = 17
      Brush.Color = clBlack
    end
    object Label13: TLabel
      Left = 768
      Top = 72
      Width = 97
      Height = 16
      Caption = 'Parcelas Pagas'
    end
    object Shape3: TShape
      Left = 744
      Top = 96
      Width = 17
      Height = 17
      Brush.Color = clBlue
    end
    object Label14: TLabel
      Left = 768
      Top = 96
      Width = 127
      Height = 16
      Caption = 'Pedidos Cancelados'
    end
    object Shape4: TShape
      Left = 744
      Top = 120
      Width = 17
      Height = 17
      Brush.Color = clMaroon
    end
    object Label15: TLabel
      Left = 768
      Top = 120
      Width = 127
      Height = 16
      Caption = 'Pedidos sem Cargas'
    end
    object Label16: TLabel
      Left = 8
      Top = 168
      Width = 62
      Height = 16
      Caption = 'N'#186' Pedido'
    end
    object Label17: TLabel
      Left = 96
      Top = 168
      Width = 55
      Height = 16
      Caption = 'N'#186' Carga'
    end
    object Label18: TLabel
      Left = 8
      Top = 116
      Width = 35
      Height = 16
      Caption = 'Usina'
    end
    object SpeedButton6: TSpeedButton
      Left = 338
      Top = 132
      Width = 23
      Height = 22
      Glyph.Data = {
        
        00FF9C948C9C948C9C948C9C948CFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      OnClick = SpeedButton6Click
    end
    object DataIni: TMaskEdit
      Left = 72
      Top = 8
      Width = 89
      Height = 22
      Ctl3D = False
      EditMask = '##/##/####'
      MaxLength = 10
      ParentCtl3D = False
      TabOrder = 0
      Text = '  /  /    '
    end
    object Datafin: TMaskEdit
      Left = 192
      Top = 8
      Width = 89
      Height = 22
      Ctl3D = False
      EditMask = '##/##/####'
      MaxLength = 10
      ParentCtl3D = False
      TabOrder = 1
      Text = '  /  /    '
    end
    object butOk: TBitBtn
      Left = 648
      Top = 181
      Width = 81
      Height = 25
      Caption = '&OK'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 16
      OnClick = butOkClick
    end
    object Cod_For: TMaskEdit
      Left = 8
      Top = 48
      Width = 49
      Height = 22
      TabStop = False
      Ctl3D = False
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 2
      Text = ''
    end
    object NomeFor: TEdit
      Left = 64
      Top = 48
      Width = 273
      Height = 22
      CharCase = ecUpperCase
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 3
      OnExit = NomeForExit
    end
    object Cod_Cliente: TMaskEdit
      Left = 376
      Top = 48
      Width = 49
      Height = 22
      TabStop = False
      Ctl3D = False
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 4
      Text = ''
    end
    object NomeCli: TEdit
      Left = 432
      Top = 48
      Width = 273
      Height = 22
      CharCase = ecUpperCase
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 5
      OnExit = NomeCliExit
    end
    object Cod_Contato: TMaskEdit
      Left = 8
      Top = 88
      Width = 49
      Height = 22
      TabStop = False
      Ctl3D = False
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 6
      Text = ''
    end
    object NomeContato: TEdit
      Left = 64
      Top = 88
      Width = 273
      Height = 22
      CharCase = ecUpperCase
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 7
      OnExit = NomeContatoExit
    end
    object Cod_Motorista: TMaskEdit
      Left = 376
      Top = 88
      Width = 49
      Height = 22
      TabStop = False
      Ctl3D = False
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 8
      Text = ''
    end
    object Nome_Motorista: TEdit
      Left = 432
      Top = 88
      Width = 273
      Height = 22
      CharCase = ecUpperCase
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 9
      OnExit = Nome_MotoristaExit
    end
    object Cod_Agencia: TMaskEdit
      Left = 192
      Top = 184
      Width = 49
      Height = 22
      TabStop = False
      Ctl3D = False
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 14
      Text = ''
      Visible = False
    end
    object Nome_Agencia: TEdit
      Left = 248
      Top = 184
      Width = 89
      Height = 22
      CharCase = ecUpperCase
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 15
      Visible = False
      OnExit = Nome_AgenciaExit
    end
    object RSituacao: TRadioGroup
      Left = 376
      Top = 168
      Width = 265
      Height = 41
      Caption = 'Situa'#231#227'o:'
      Columns = 3
      ItemIndex = 2
      Items.Strings = (
        'Aberto'
        'Encerrado'
        'Todos')
      TabOrder = 17
      Visible = False
    end
    object Num_Pedido: TMaskEdit
      Left = 8
      Top = 184
      Width = 73
      Height = 22
      Ctl3D = False
      EditMask = '########;-;_'
      MaxLength = 8
      ParentCtl3D = False
      TabOrder = 12
      Text = '        '
    end
    object NumCarga: TMaskEdit
      Left = 96
      Top = 184
      Width = 73
      Height = 22
      Ctl3D = False
      EditMask = '########;-;_'
      MaxLength = 8
      ParentCtl3D = False
      TabOrder = 13
      Text = '        '
    end
    object chkCancelados: TCheckBox
      Left = 8
      Top = 218
      Width = 201
      Height = 17
      Caption = 'Mostrar Pedidos Cancelados'
      TabOrder = 18
    end
    object Cod_Usina: TEdit
      Left = 8
      Top = 131
      Width = 49
      Height = 22
      TabStop = False
      Ctl3D = False
      NumbersOnly = True
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 10
    end
    object Nome_Usina: TEdit
      Left = 63
      Top = 131
      Width = 274
      Height = 22
      CharCase = ecUpperCase
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 11
      OnExit = Nome_UsinaExit
    end
  end
  object Grade: TDBGrid
    Left = 0
    Top = 243
    Width = 878
    Height = 191
    Align = alClient
    DataSource = dsLista
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
    PopupMenu = P_MenuAux
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnDrawColumnCell = GradeDrawColumnCell
    Columns = <
      item
        Expanded = False
        FieldName = 'NOME_FOR'
        Title.Caption = 'Fornecedor'
        Width = 100
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NOME_USINA'
        Title.Caption = 'Usina'
        Width = 100
        Visible = True
      end
      item
        Color = clSkyBlue
        Expanded = False
        FieldName = 'NOME_CLIENTE'
        Title.Caption = 'Cliente'
        Width = 156
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NOME_CONTATO'
        Title.Caption = 'Contato'
        Width = 136
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NUM_PEDIDO'
        Title.Caption = 'N'#186' Ped.'
        Width = 50
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PRECO_CUSTO'
        Title.Caption = 'P.Ped'
        Width = 56
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PRECO_VENDA'
        Title.Caption = 'P.Venda'
        Width = 56
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'TOTAL_QTDE'
        Title.Caption = 'Quant.'
        Width = 50
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NUM_CARGA'
        Title.Caption = 'N'#186' Carga'
        Width = 57
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'LETRA'
        Title.Caption = 'Letra'
        Width = 33
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VALOR_CARREGA'
        Title.Caption = 'Diferen'#231'a'
        Width = 63
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'COMPLEMENTO'
        Title.Caption = 'Obs'
        Width = 29
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'C_VALORPAGO'
        Title.Caption = 'Vlr.Recebido'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'C_NUM_CONTA'
        Title.Caption = 'N'#186'Conta'
        Width = 94
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DATA'
        Title.Caption = 'Data Pedido'
        Visible = True
      end>
  end
  object Panel2: TPanel
    Left = 0
    Top = 434
    Width = 878
    Height = 49
    Align = alBottom
    BevelOuter = bvLowered
    TabOrder = 2
    object Label3: TLabel
      Left = 40
      Top = 16
      Width = 78
      Height = 16
      Caption = 'Total Cargas'
    end
    object Label5: TLabel
      Left = 280
      Top = 16
      Width = 50
      Height = 16
      Caption = 'Total Dif'
    end
    object Label4: TLabel
      Left = 472
      Top = 16
      Width = 83
      Height = 16
      Caption = 'Qtde Pedidos'
    end
    object Label19: TLabel
      Left = 706
      Top = 14
      Width = 50
      Height = 16
      Caption = 'Qtde KG'
    end
    object DBEdit1: TDBEdit
      Left = 128
      Top = 8
      Width = 121
      Height = 22
      TabStop = False
      Color = clBtnFace
      Ctl3D = False
      DataField = 'SOMA_CARGAS'
      DataSource = dsLista
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 0
    end
    object DBEdit3: TDBEdit
      Left = 336
      Top = 8
      Width = 121
      Height = 22
      TabStop = False
      Color = clBtnFace
      Ctl3D = False
      DataField = 'SOMA_LUCRO'
      DataSource = dsLista
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 1
    end
    object QtdePedidos: TEdit
      Left = 560
      Top = 8
      Width = 105
      Height = 22
      Color = clBtnFace
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 2
    end
    object edtQtdeKilos: TEdit
      Left = 768
      Top = 6
      Width = 105
      Height = 22
      Color = clBtnFace
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 3
    end
  end
  object QLista: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'Select'
      '   FO.NOME AS NOME_FOR,'
      '   CLI.NOME AS NOME_CLIENTE,'
      '   CON.NOME AS NOME_CONTATO,'
      '   PED.TOTAL_LIQUIDO,'
      '   PED.COD_CLIENTE,'
      '   PED.DATA,'
      '   PED.TOTAL_VENDA,'
      '   PED.TOTAL_QTDE,'
      '   UN.FATOR_CONVERSAO,'
      
        '   (COALESCE(PED.TOTAL_QTDE, 0) / COALESCE(UN.fator_conversao,1)' +
        ') AS FATOR_CONVERSAO_QTDE,'
      '   PED.SITUACAO AS PED_ENCERRADO,'
      '      case'
      '    when PED.TOTAL_QTDE > 0 then'
      '        PED.TOTAL_LIQUIDO / PED.TOTAL_QTDE'
      '    else'
      '        0'
      '   end as PRECO_CUSTO,'
      ''
      '   case'
      '    when PED.TOTAL_QTDE > 0 then'
      '        PED.TOTAL_VENDA / PED.TOTAL_QTDE'
      '    else'
      '        0'
      '   end  AS PRECO_VENDA,'
      ''
      '   case'
      '    when PED.TOTAL_QTDE > 0 then'
      '        PED.TOTAL_LUCRO / PED.TOTAL_QTDE'
      '    else'
      '        0'
      '   end  AS VALOR_LUCRO,'
      '   PED.NUM_PEDIDO,'
      '   CAR.NUM_CARGA,'
      '   CAR.VALOR_FRETE,'
      '   CAR.VALOR_CARREGA,'
      '   CAR.SITUACAO,'
      '   CAR.FINANCEIRO,'
      '   CAR.COMPLEMENTO,'
      '   CAR.ID_CARGA,'
      '   CAR.LETRA,'
      '   USI.nome AS NOME_USINA'
      'from PEDIDO PED'
      '   left join CARGA CAR on PED.NUM_PEDIDO = CAR.NUM_PEDIDO'
      
        '   left join ITENS_PEDIDO IPED on PED.num_pedido = IPED.num_pedi' +
        'do'
      '   left join UNIDADE UN on IPED.id_unidade = UN.id_unidade'
      '   left join FORNECEDOR FO on PED.COD_FOR = FO.COD_FOR'
      '   left join CLIENTE CLI on PED.COD_CLIENTE = CLI.COD_CLIENTE'
      '   left join CLIENTE CON on CAR.COD_CONTATO = CON.COD_CLIENTE'
      '   left join FORNECEDOR USI on PED.cod_usina = USI.cod_for')
    SQLConnection = DM.BancoDados
    Left = 168
    Top = 288
  end
  object dspLista: TDataSetProvider
    DataSet = QLista
    Left = 224
    Top = 288
  end
  object Lista: TClientDataSet
    Aggregates = <>
    AggregatesActive = True
    Params = <>
    ProviderName = 'dspLista'
    OnCalcFields = ListaCalcFields
    Left = 288
    Top = 288
    object ListaNOME_FOR: TStringField
      FieldName = 'NOME_FOR'
      Size = 60
    end
    object ListaNOME_CLIENTE: TStringField
      FieldName = 'NOME_CLIENTE'
      Size = 60
    end
    object ListaNOME_CONTATO: TStringField
      FieldName = 'NOME_CONTATO'
      Size = 60
    end
    object ListaSITUACAO: TStringField
      FieldName = 'SITUACAO'
      FixedChar = True
      Size = 1
    end
    object ListaNUM_PEDIDO: TIntegerField
      FieldName = 'NUM_PEDIDO'
    end
    object ListaID_CARGA: TIntegerField
      FieldName = 'ID_CARGA'
    end
    object ListaTOTAL_LIQUIDO: TFloatField
      FieldName = 'TOTAL_LIQUIDO'
      DisplayFormat = ',##0.00'
      currency = True
    end
    object ListaTOTAL_VENDA: TFloatField
      FieldName = 'TOTAL_VENDA'
      DisplayFormat = ',##0.00'
      currency = True
    end
    object ListaTOTAL_QTDE: TFloatField
      FieldName = 'TOTAL_QTDE'
      DisplayFormat = ',##0'
    end
    object ListaNUM_CARGA: TIntegerField
      FieldName = 'NUM_CARGA'
    end
    object ListaLETRA: TStringField
      FieldName = 'LETRA'
      FixedChar = True
      Size = 2
    end
    object ListaPRECO_CUSTO: TFloatField
      FieldName = 'PRECO_CUSTO'
      DisplayFormat = ',##0.00'
      currency = True
    end
    object ListaPRECO_VENDA: TFloatField
      FieldName = 'PRECO_VENDA'
      DisplayFormat = ',##0.00'
      currency = True
    end
    object ListaVALOR_LUCRO: TFloatField
      FieldName = 'VALOR_LUCRO'
      DisplayFormat = ',##0.00'
      currency = True
    end
    object ListaVALOR_FRETE: TFloatField
      FieldName = 'VALOR_FRETE'
      DisplayFormat = ',##0.00'
      currency = True
    end
    object ListaPED_ENCERRADO: TStringField
      FieldName = 'PED_ENCERRADO'
      FixedChar = True
      Size = 1
    end
    object ListaDATA: TSQLTimeStampField
      FieldName = 'DATA'
    end
    object ListaVALOR_CARREGA: TFloatField
      FieldName = 'VALOR_CARREGA'
      DisplayFormat = ',##0.00'
    end
    object ListaFINANCEIRO: TStringField
      FieldName = 'FINANCEIRO'
      FixedChar = True
      Size = 1
    end
    object ListaCOD_CLIENTE: TIntegerField
      FieldName = 'COD_CLIENTE'
      Required = True
    end
    object ListaC_VALORPAGO: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'C_VALORPAGO'
      DisplayFormat = ',##0.00'
    end
    object ListaC_NUM_CONTA: TStringField
      FieldKind = fkInternalCalc
      FieldName = 'C_NUM_CONTA'
    end
    object ListaCOMPLEMENTO: TStringField
      FieldName = 'COMPLEMENTO'
      Size = 500
    end
    object ListaNOME_USINA: TStringField
      FieldName = 'NOME_USINA'
      Size = 60
    end
    object ListaFATOR_CONVERSAO_QTDE: TFloatField
      FieldName = 'FATOR_CONVERSAO_QTDE'
    end
    object ListaFATOR_CONVERSAO: TFMTBCDField
      FieldName = 'FATOR_CONVERSAO'
      Precision = 18
      Size = 6
    end
    object ListaSOMA_CARGAS: TAggregateField
      Alignment = taRightJustify
      FieldName = 'SOMA_CARGAS'
      Active = True
      DisplayName = ''
      Expression = 'count(NUM_CARGA)'
    end
    object ListaSOMA_LUCRO: TAggregateField
      Alignment = taRightJustify
      FieldName = 'SOMA_LUCRO'
      Active = True
      currency = True
      DisplayName = ''
      Expression = 'sum(VALOR_CARREGA)'
    end
    object ListaSOMA_FRETE: TAggregateField
      Alignment = taRightJustify
      FieldName = 'SOMA_FRETE'
      Active = True
      currency = True
      DisplayName = ''
      Expression = 'sum(VALOR_FRETE)'
    end
  end
  object dsLista: TDataSource
    DataSet = Lista
    Left = 344
    Top = 288
  end
  object P_MenuAux: TPopupMenu
    Left = 464
    Top = 288
    object CargaOk: TMenuItem
      Caption = 'Ok Carga'
      Visible = False
      OnClick = CargaOkClick
    end
    object Parcelas: TMenuItem
      Caption = 'Ver Diferen'#231'a'
      OnClick = ParcelasClick
    end
    object VerPedido: TMenuItem
      Caption = 'Ver Pedido'
      OnClick = VerPedidoClick
    end
    object VerCarga: TMenuItem
      Caption = 'Ver Carga'
      OnClick = VerCargaClick
    end
    object VerCondicoes: TMenuItem
      Caption = 'Ver Codi'#231#245'es de Pagto'
      OnClick = VerCondicoesClick
    end
    object FinanceiroOk: TMenuItem
      Caption = 'Ok Financeiro'
      Visible = False
      OnClick = FinanceiroOkClick
    end
    object Obs: TMenuItem
      Caption = 'Observa'#231#227'o'
      OnClick = ObsClick
    end
  end
  object Qcontas: TSQLDataSet
    CommandText = 
      'select '#13#10'BAN.NUM_CONTA,'#13#10'sum(CON.VALOR_PAGO) as VALORPAGO'#13#10'from ' +
      'CONTAS CON'#13#10' left join CONTABANCO BAN on CON.ID_CONTABANCO = BAN' +
      '.ID_CONTABANCO'#13#10' where CON.NUM_PEDIDO = :NUM_PEDIDO'#13#10' and CON.TI' +
      'PO_CONTA = 2'#13#10' group by BAN.NUM_CONTA'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'NUM_PEDIDO'
        ParamType = ptInput
      end>
    SQLConnection = DM.BancoDados
    Left = 336
    Top = 352
    object QcontasVALORPAGO: TFloatField
      FieldName = 'VALORPAGO'
    end
    object QcontasNUM_CONTA: TStringField
      FieldName = 'NUM_CONTA'
    end
  end
  object QListaAux: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'Select'
      '   FO.NOME AS NOME_FOR,'
      '   CLI.NOME AS NOME_CLIENTE,'
      '   CON.NOME AS NOME_CONTATO,'
      '   PED.TOTAL_LIQUIDO,'
      '   PED.COD_CLIENTE,'
      '   PED.DATA,'
      '   PED.TOTAL_VENDA,'
      '   PED.TOTAL_QTDE,'
      '   UN.FATOR_CONVERSAO,'
      
        '   (COALESCE(PED.TOTAL_QTDE, 0) / COALESCE(UN.fator_conversao,0)' +
        ') AS FATOR_CONVERSAO_QTDE,'
      '   PED.SITUACAO AS PED_ENCERRADO,'
      '      case'
      '    when PED.TOTAL_QTDE > 0 then'
      '        PED.TOTAL_LIQUIDO / PED.TOTAL_QTDE'
      '    else'
      '        0'
      '   end as PRECO_CUSTO,'
      ''
      '   case'
      '    when PED.TOTAL_QTDE > 0 then'
      '        PED.TOTAL_VENDA / PED.TOTAL_QTDE'
      '    else'
      '        0'
      '   end  AS PRECO_VENDA,'
      ''
      '   case'
      '    when PED.TOTAL_QTDE > 0 then'
      '        PED.TOTAL_LUCRO / PED.TOTAL_QTDE'
      '    else'
      '        0'
      '   end  AS VALOR_LUCRO,'
      '   PED.NUM_PEDIDO,'
      '   CAR.NUM_CARGA,'
      '   CAR.VALOR_FRETE,'
      '   CAR.VALOR_CARREGA,'
      '   CAR.SITUACAO,'
      '   CAR.FINANCEIRO,'
      '   CAR.COMPLEMENTO,'
      '   CAR.ID_CARGA,'
      '   CAR.LETRA,'
      '   USI.nome AS NOME_USINA'
      'from PEDIDO PED'
      '   left join CARGA CAR on PED.NUM_PEDIDO = CAR.NUM_PEDIDO'
      
        '   left join ITENS_PEDIDO IPED on PED.num_pedido = IPED.num_pedi' +
        'do'
      '   left join UNIDADE UN on IPED.id_unidade = UN.id_unidade'
      '   left join FORNECEDOR FO on PED.COD_FOR = FO.COD_FOR'
      '   left join CLIENTE CLI on PED.COD_CLIENTE = CLI.COD_CLIENTE'
      '   left join CLIENTE CON on CAR.COD_CONTATO = CON.COD_CLIENTE'
      '   left join FORNECEDOR USI on PED.cod_usina = USI.cod_for')
    SQLConnection = DM.BancoDados
    Left = 480
    Top = 192
  end
end
